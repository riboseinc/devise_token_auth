...
module DeviseTokenAuth::Concerns::ResourceFinder
  ...
  def find_resource(field, value)
    @resource = if dta_resource_class.try(:connection_config).try(:[], :adapter).try(:include?, 'mysql')
                  # fix for mysql default case insensitivity
                  dta_resource_class.where("BINARY #{field} = ? AND provider= ?", value, provider).first
                else
